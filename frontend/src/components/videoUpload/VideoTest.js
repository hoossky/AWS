...
const VideoTest = () => {
    ...
    const handleSubmit = () => {
        ...
        if (strSelectedDay === "") {
            ...
            axios
                .patch(`http://ip/orders/modifyform/${userId}`, estiJsnon)
                .then(response => {
                    alert('성공');
    ...
    useEffect(() => {
        axios
            .get(`http://ip/file/getfilename/${JSON.parse(localStorage.getItem("estiDate")).orderId}`)
            .then(({data}) => {
                setImageList(data);
